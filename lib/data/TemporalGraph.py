...
class TemporalGraph(InMemoryDataset):
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            data = self.read_TGraph_data(self.root, self.tw_id)
    ...
    def read_TGraph_data(self, dir, tw_id, b_sparse = True):
        ...
        if tw_id == "all": # Processing aggregated static graph inputs
            ...
            for line in f.readlines():
                ...
                graph[s].append([d])
            f.close()
            edge_index = self.edge_index_from_dict(graph, num_nodes=num_nodes) # torch.Size([2, 61470])
            # Step 2: get attr
            x = None
            if b_sparse:
                x_loc = [[], []]
                ...
                for n, edges in graph.items():
                    for edge in edges:
                        ...
                        x_val.append(1) # DEFAULT VALUE 1
...
if __name__ == '__main__':
    dataset = TemporalGraph(root='src/IMDB/imdb', name='imdb', tw_id='all', b_nodeattr=True)
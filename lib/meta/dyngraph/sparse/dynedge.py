...
class DynamicAdjacencyListDynamicEdge(DynamicGraph):
    ...
    def set_win_shift(self, num: int, /, data_source=None) -> None:
        R"""
        Change window shifting factor.
        """
        # Should only support DynCSL.
        # if data_source is not None and data_source == 'UIHC':
        #     self.WIN_SHIFT = num
        if num != 8:
            # UNEXPECT:
            # Only support DynCSL.
    ...
    def pin(self, batch_size: int, /) -> MEMPIN:
        ...
        if (
            ...
            self.node_feat_size == 73
            and self.num_times == 61
        ):
            #
            corner_case = "IMDB"
        elif (
            self.node_feat_size == 1
            and self.num_times == 31
        ):
            #
            corner_case = "UIHC"
        else:

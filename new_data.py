...
# convert timestamps to corresponding second 
# dts = [datetime.fromtimestamp(ts) for ts in full_data.timestamps]

# plot edge count distribution over time
plt.figure()
bin1 = plt.hist(full_data.timestamps, bins=200)
plt.show()
# split timestamps into fixed number of bins using numpy
import numpy as np
bin2 = np.linspace(min(full_data.timestamps), max(full_data.timestamps), 201)
